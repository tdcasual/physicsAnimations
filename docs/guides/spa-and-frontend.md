...
- `/` 为导航型首页，结构为：英雄区 → 快捷入口 → 分组导航 → 当前分类 / 推荐演示 / 资源库精选 → 完整列表。
- 目录页保留搜索和分组/分类上下文；移动端通过“筛选导航”按钮打开受控筛选面板，而不是只依赖横向 tab。
- 公开壳层顶部会保留品牌说明与返回首页入口，便于从预览页或管理页回到导航首页。

### 管理端

- `/admin/*` 使用工作区壳层：顶部显示当前模块说明，侧边/顶部提供分组导航。
- 导航分为“内容管理 / 资源结构 / 系统设置”三组，减少单排标签的扫描负担。
- 小屏设备下导航折叠为“工作区菜单”，展开后仍保留横向滚动与焦点滚动可达性。
