...
test.describe('目录浏览', () => {
  ...
  test('应显示分类标签', async ({ page }) => {
    ...
    const allButton = page.locator('.nav-categories button:has-text("全部"), button:has-text("All")').first()
  ...
  test('应能切换分类', async ({ page }) => {
    // 找到第一个非"全部"的分类按钮
    const categoryButtons = page.locator('.nav-categories button:not(.active), .nav-categories .nav-tab:not(.active)')
  ...
  test('搜索功能应正常工作', async ({ page }) => {
    const searchInput = page.locator('.topbar-search, input[type="search"], input[placeholder*="搜索"]').first()
