...
export function createAdminTaxonomyMobileEditorFocus(
  params: CreateAdminTaxonomyMobileEditorFocusParams
) {
  const {
    editorTopRef,
    groupEditorRef,
    categoryEditorRef,
    isMobileViewport = isMobileTaxonomyViewport,
  } = params
  ...
  async function focusEditorTarget(target: AdminTaxonomyEditorFocusTarget) {
    ...
    const destination =
      (target === 'group' ? groupEditorRef.value : categoryEditorRef.value) ?? editorTopRef.value
