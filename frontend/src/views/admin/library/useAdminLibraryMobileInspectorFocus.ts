...
export function createAdminLibraryMobileInspectorFocus(params: CreateAdminLibraryMobileInspectorFocusParams) {
  const { inspectorTopRef, folderMetaSectionRef, assetEditSectionRef, isMobileViewport = isMobileLibraryViewport } = params;
  ...
  async function focusInspectorTarget(target: AdminLibraryInspectorFocusTarget) {
    ...
    const destination =
      (target === "folder" ? folderMetaSectionRef.value : assetEditSectionRef.value) ?? inspectorTopRef.value;
