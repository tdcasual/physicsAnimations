...
export function useLibraryAssetFilters(deps: UseLibraryAssetFiltersDeps) {
  ...
  const selectableEmbedProfiles = computed(() =>
    deps.embedProfiles.value.filter(profile => profile.enabled !== false)
  )
  ...
  const sortedFilteredFolderAssets = computed(() => {
    ...
    list.sort((a, b) => {
      if (assetSortMode.value === 'name_asc') {
        return safeText(a.displayName || a.fileName).localeCompare(
          safeText(b.displayName || b.fileName)
        )
      }
      if (assetSortMode.value === 'name_desc') {
        return safeText(b.displayName || b.fileName).localeCompare(
          safeText(a.displayName || a.fileName)
        )
      }
