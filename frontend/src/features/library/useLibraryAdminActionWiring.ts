...
export function useLibraryAdminActionWiring(params: UseLibraryAdminActionWiringParams) {
  ...
  const editingEmbedProfile = computed(
    () => draft.embedProfiles.value.find((profile) => profile.id === draft.editingEmbedProfileId.value) || null,
  );
  ...
  const hasPendingFolderEditChanges = computed(() => {
    ...
    const baselineCategoryId = folder.categoryId || folderActions.groupedCategoryOptions.value[0]?.value || "other";
    return (
  ...
  const hasPendingEmbedEditChanges = computed(() => {
    ...
    return (
      ...
      draft.embedEditExtensionsText.value !== (Array.isArray(profile.matchExtensions) ? profile.matchExtensions.join(",") : "") ||
      draft.embedEditDefaultOptionsJson.value !== JSON.stringify(profile.defaultOptions || {}, null, 2) ||
  ...
  function selectFolder(folderId: string, options: { panelTab?: LibraryPanelTab } = {}) {
    ...
    if (hasPendingFolderSwitchChanges.value) {
      if (!confirmDiscardPendingChanges("当前资源库编辑内容有未保存更改，确定切换文件夹吗？")) return;
    }
