...
export function useLibraryAdminDataActions(deps: LibraryAdminDataActionsDeps) {
  async function reloadFolders() {
    ...
    if (!deps.selectedFolderId.value && list.length > 0) {
      ...
      deps.selectedFolderId.value &&
      !list.some(folder => folder.id === deps.selectedFolderId.value)
    ) {
      deps.selectedFolderId.value = list[0]?.id || ''
    }
  ...
  async function reloadFolderAssets() {
    ...
    try {
      ...
      if (requestId !== deps.folderAssetsLoadSeq.value || deps.selectedFolderId.value !== folderId)
        return
      ...
      deps.selectedAssetIds.value = deps.selectedAssetIds.value.filter(id =>
        folderAssetIdSet.has(id)
      )
