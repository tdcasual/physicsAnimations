...
export function buildUpdateLibraryFolderBody(
...
export function buildCreateLibraryEmbedProfileBody(payload: CreateLibraryEmbedProfilePayload) {
  return {
    ...
    defaultOptions:
      ...
      typeof payload.defaultOptions === 'object' &&
      !Array.isArray(payload.defaultOptions)
        ? payload.defaultOptions
        : {},
...
export function buildUpdateLibraryEmbedProfileBody(
  patch: UpdateLibraryEmbedProfilePatch
): Record<string, unknown> {
  const body: Record<string, unknown> = {}
  if (patch.name !== undefined) body.name = String(patch.name || '').trim()
  if (patch.scriptUrl !== undefined) body.scriptUrl = String(patch.scriptUrl || '').trim()
  if (patch.fallbackScriptUrl !== undefined)
    body.fallbackScriptUrl = String(patch.fallbackScriptUrl || '').trim()
  if (patch.viewerPath !== undefined) body.viewerPath = String(patch.viewerPath || '').trim()
  if (patch.constructorName !== undefined)
    body.constructorName = String(patch.constructorName || '').trim()
  if (patch.assetUrlOptionKey !== undefined)
    body.assetUrlOptionKey = String(patch.assetUrlOptionKey || '').trim()
  if (patch.matchExtensions !== undefined)
    body.matchExtensions = Array.isArray(patch.matchExtensions) ? patch.matchExtensions : []
  if (patch.defaultOptions !== undefined) {
    body.defaultOptions =
      patch.defaultOptions &&
      typeof patch.defaultOptions === 'object' &&
      !Array.isArray(patch.defaultOptions)
        ? patch.defaultOptions
        : {}
  }
...
export function buildUpdateLibraryAssetBody(
  ...
  if (patch?.embedOptions !== undefined) {
    body.embedOptions =
      patch.embedOptions &&
      typeof patch.embedOptions === 'object' &&
      !Array.isArray(patch.embedOptions)
        ? patch.embedOptions
        : {}
  }
  return body
