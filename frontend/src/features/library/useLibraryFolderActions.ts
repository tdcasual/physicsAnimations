...
export function useLibraryFolderActions(deps: UseLibraryFolderActionsDeps) {
  ...
  function syncFolderEditDraft() {
    ...
    folderEditCategoryId.value =
      folder.categoryId || groupedCategoryOptions.value[0]?.value || 'other'
  }
  ...
  async function createFolderEntry() {
    ...
    try {
      ...
      const createCoverInput = document.querySelector<HTMLInputElement>(
        '#library-create-cover-file'
      )
