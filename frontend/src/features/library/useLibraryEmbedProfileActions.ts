...
export function useLibraryEmbedProfileActions(deps: UseLibraryEmbedProfileActionsDeps) {
  ...
  async function reloadEmbedProfiles() {
    ...
    if (
      ...
      !list.some(
        profile => profile.id === deps.assetEmbedProfileId.value && profile.enabled !== false
      )
    ) {
    ...
    if (
      ...
      (!deps.assetEditEmbedProfileId.value ||
        !list.some(
          profile => profile.id === deps.assetEditEmbedProfileId.value && profile.enabled !== false
        ))
    ) {
    ...
    if (
      deps.editingEmbedProfileId.value &&
      !list.some(profile => profile.id === deps.editingEmbedProfileId.value)
    ) {
