...
function serializeEmbedUpdaterDraft(draft: {
  enabled: boolean
...
export function createSystemWizardBindings(params: SystemWizardBindingsParams) {
  function buildStorageSnapshot(): string {
    return serializeStorageDraft({
      ...
      timeoutMs: Number.isFinite(params.timeoutMs.value)
        ? Math.trunc(params.timeoutMs.value)
        : null,
  ...
  function applyEmbedUpdater(nextEmbedUpdater: any) {
    const summary =
      nextEmbedUpdater?.lastSummary && typeof nextEmbedUpdater.lastSummary === 'object'
        ? nextEmbedUpdater.lastSummary
        : {}
    ...
    params.embedUpdater.value = {
      ...
      lastSummary: {
        ...
        totalProfiles: Number.isFinite(summary?.totalProfiles)
          ? Math.trunc(summary.totalProfiles)
          : 0,
        syncedProfiles: Number.isFinite(summary?.syncedProfiles)
          ? Math.trunc(summary.syncedProfiles)
          : 0,
        skippedProfiles: Number.isFinite(summary?.skippedProfiles)
          ? Math.trunc(summary.skippedProfiles)
          : 0,
        failedProfiles: Number.isFinite(summary?.failedProfiles)
          ? Math.trunc(summary.failedProfiles)
          : 0,
      },
    }
