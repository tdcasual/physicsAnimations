...
function serializeItemSnapshot(
  item: EditableAdminItem | null | undefined,
  defaultCategoryId: string
): string {
  if (!item) return ''
...
export function createAdminItemEditorState<T extends EditableAdminItem>(
  params: CreateAdminItemEditorStateParams<T>
) {
  const editingId = ref('')
  const editTitle = ref('')
  ...
  const loadedEditSnapshot = computed(() =>
    serializeItemSnapshot(editingSnapshot.value, params.defaultCategoryId)
  )
  const hasPendingEditChanges = computed(() =>
    editingSnapshot.value ? buildEditSnapshot() !== loadedEditSnapshot.value : false
  )
