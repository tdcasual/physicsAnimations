...
export function useTaxonomyAdmin() {
  ...
  function syncSelectionAndOpenGroups() {
    ...
    openGroupIds.value = [
      ...new Set(openGroupIds.value.map(id => String(id || '').trim()).filter(Boolean)),
  ...
  function groupMetaText(node: { group: GroupRow; shownCategories: CategoryRow[] }): string {
    ...
    const shownItems = node.shownCategories.reduce(
    ...
    const categoryText =
      totalCategories && shownCategories !== totalCategories
        ...
        : `分类 ${shownCategories}`
    const itemText =
      totalItems && shownItems !== totalItems
        ? `内容 ${shownItems}/${totalItems}`
        : `内容 ${totalItems || shownItems}`
  ...
  const hasPendingChanges = computed(() => {
    ...
    return (
      hasGroupEditChanges ||
      hasCreateGroupChanges ||
      hasCreateCategoryChanges ||
      hasCategoryEditChanges
    )
  })
