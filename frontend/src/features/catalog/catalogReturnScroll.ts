...
export function parseCatalogReturnScroll(raw: string | null | undefined): CatalogReturnScrollSnapshot | null {
  try {
    if (!raw) return null;
...
export function resolveCatalogReturnScrollRestore(input: {
  ...
  const forward =
    historyState && typeof historyState === "object" && typeof (historyState as Record<string, unknown>).forward === "string"
