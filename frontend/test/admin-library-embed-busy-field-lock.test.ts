...
describe('admin library embed busy field locking', () => {
  it('locks embed create form fields while create/save is in flight', () => {
    ...
    expect(source).toMatch(
      /<input(?:(?!\/>)[\s\S])*v-model="vm\.drafts\.embedProfileName"(?:(?!\/>)[\s\S])*:disabled="vm\.ui\.savingEmbed"(?:(?!\/>)[\s\S])*\/>/
    )
    expect(source).toMatch(
      /<input(?:(?!\/>)[\s\S])*v-model="vm\.drafts\.embedScriptUrl"(?:(?!\/>)[\s\S])*:disabled="vm\.ui\.savingEmbed"(?:(?!\/>)[\s\S])*\/>/
    )
    expect(source).toMatch(
      /<textarea(?:(?!\/>)[\s\S])*v-model="vm\.drafts\.embedDefaultOptionsJson"(?:(?!\/>)[\s\S])*:disabled="vm\.ui\.savingEmbed"(?:(?!\/>)[\s\S])*\/>/
    )
    expect(source).toMatch(
      /<input(?:(?!\/>)[\s\S])*v-model="vm\.drafts\.embedEnabled"(?:(?!\/>)[\s\S])*:disabled="vm\.ui\.savingEmbed"(?:(?!\/>)[\s\S])*\/>/
    )
    expect(source).toMatch(
  ...
  it('locks embed edit form fields while save is in flight', () => {
    ...
    expect(source).toMatch(
      /<input(?:(?!\/>)[\s\S])*v-model="vm\.drafts\.embedEditName"(?:(?!\/>)[\s\S])*:disabled="vm\.ui\.savingEmbed"(?:(?!\/>)[\s\S])*\/>/
    )
    expect(source).toMatch(
      /<input(?:(?!\/>)[\s\S])*v-model="vm\.drafts\.embedEditScriptUrl"(?:(?!\/>)[\s\S])*:disabled="vm\.ui\.savingEmbed"(?:(?!\/>)[\s\S])*\/>/
    )
    expect(source).toMatch(
      /<textarea(?:(?!\/>)[\s\S])*v-model="vm\.drafts\.embedEditDefaultOptionsJson"(?:(?!\/>)[\s\S])*:disabled="vm\.ui\.savingEmbed"(?:(?!\/>)[\s\S])*\/>/
    )
    expect(source).toMatch(
      /<input(?:(?!\/>)[\s\S])*v-model="vm\.drafts\.embedEditEnabled"(?:(?!\/>)[\s\S])*:disabled="vm\.ui\.savingEmbed"(?:(?!\/>)[\s\S])*\/>/
    )
    expect(source).toMatch(
