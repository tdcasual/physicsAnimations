...
describe('viewer action bar', () => {
  ...
  it('tears down the iframe when switching external previews back to screenshot mode', () => {
    const source = read('src/views/ViewerView.vue')
    expect(source).toMatch(
      /function toggleMode\([\s\S]*if \(screenshotMode\.value\) \{[\s\S]*interactiveStarted\.value = true/
    )
    expect(source).toMatch(
      /function toggleMode\([\s\S]*else \{[\s\S]*interactiveStarted\.value = false/
    )
  })
  ...
  it('tightens the compact viewer header on small screens so the stage arrives sooner', () => {
    const source = read('src/views/ViewerView.vue')
    expect(source).toMatch(
      /@media \(max-width: 640px\)\s*\{[\s\S]*\.viewer-bar--compact\s*\{[\s\S]*padding:\s*10px 12px/
    )
    expect(source).toMatch(
      /@media \(max-width: 640px\)\s*\{[\s\S]*\.viewer-bar--compact\s*\{[\s\S]*gap:\s*8px/
    )
    expect(source).toMatch(
      /@media \(max-width: 640px\)\s*\{[\s\S]*\.viewer-bar-left\s*\{[\s\S]*gap:\s*8px/
    )
    expect(source).toMatch(/\.viewer-back\s*\{[\s\S]*white-space:\s*nowrap/)
    expect(source).toMatch(
      /@media \(max-width: 640px\)\s*\{[\s\S]*\.viewer-title\s*\{[\s\S]*font-size:\s*clamp\(1\.05rem,\s*0\.95rem \+ 0\.4vw,\s*1\.3rem\)/
    )
  })
  ...
  it('switches viewer pages into a more immersive mobile shell with tighter shared chrome', () => {
    ...
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.topbar--viewer\s+\.topbar-search-launch\s*\{[\s\S]*display:\s*none/
    )
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.topbar--viewer\s+\.topbar-search-field\s*\{[\s\S]*display:\s*none/
    )
    expect(viewerSource).toMatch(
      /@media \(max-width: 640px\)\s*\{[\s\S]*\.viewer-actions\s*\{[\s\S]*display:\s*grid/
    )
    expect(viewerSource).toMatch(
      /@media \(max-width: 640px\)\s*\{[\s\S]*\.viewer-actions\s*\{[\s\S]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/
    )
    expect(viewerSource).toMatch(
      /@media \(max-width: 640px\)\s*\{[\s\S]*\.viewer-stage-screen\s*\{[\s\S]*min-height:\s*min\(/
    )
  })
})
