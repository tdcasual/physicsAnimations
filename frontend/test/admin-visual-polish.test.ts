...
describe('admin visual polish', () => {
  ...
  it('turns the mobile workspace menu into a bottom-sheet control deck', () => {
    ...
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-nav-bar\s*\{[\s\S]*position:\s*fixed[\s\S]*left:\s*12px[\s\S]*right:\s*12px[\s\S]*bottom:\s*12px/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-nav-bar\.is-open\s*\{[\s\S]*display:\s*grid[\s\S]*animation:/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-nav-sheet-heading\s*\{[\s\S]*display:\s*flex/
    )
  })
  ...
  it('compresses repeated admin shell copy on mobile into an app-bar plus quick section strip', () => {
    ...
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-shell-description\s*\{[\s\S]*display:\s*none/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-shell-note\s*\{[\s\S]*display:\s*none/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-shell-header\s*\{[\s\S]*border:\s*0[\s\S]*background:\s*transparent/
    )
    expect(source).toMatch(
    ...
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-shell-copy\s*\{[\s\S]*gap:\s*4px/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-shell-header\s*\{[\s\S]*padding:\s*2px\s*2px\s*0/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-shell-ops\s*\{[\s\S]*display:\s*flex[\s\S]*justify-content:\s*flex-end/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-shell-status-strip\s*\{[\s\S]*display:\s*none/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-mobile-nav-strip\s*\{[\s\S]*position:\s*sticky[\s\S]*top:\s*calc\(var\(--app-topbar-height,\s*0px\)\s*\+\s*8px\)/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-mobile-nav-summary\s*\{[\s\S]*display:\s*none/
    )
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-mobile-nav-links\s*\{[\s\S]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/
    )
    expect(source).toMatch(
  ...
  it('treats dashboard, content, and uploads headers as compact operational framing on mobile', () => {
    ...
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-intro--supporting\s*\{[\s\S]*display:\s*none/
    )
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-meta-copy\s*\{[\s\S]*display:\s*none/
    )
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-header\s*\{[\s\S]*padding:\s*12px\s*12px\s*10px/
    )
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-meta\s*\{[\s\S]*display:\s*flex[\s\S]*justify-content:\s*space-between/
    )
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-meta-list\s*\{[\s\S]*display:\s*none/
    )
  })
  ...
  it('compacts the dashboard lead task and create forms without removing optional metadata access', () => {
    ...
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-task-copy--supporting\s*\{[\s\S]*display:\s*none/
    )
  })
  ...
  it('removes redundant first-screen label rows on mobile so the next admin card appears sooner', () => {
    ...
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-kicker\s*\{[\s\S]*display:\s*none/
    )
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-meta-label\s*\{[\s\S]*display:\s*none/
    )
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-copy\s*\{[\s\S]*gap:\s*4px/
    )
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-meta\s*\{[\s\S]*padding:\s*8px\s*10px/
    )
  })
  ...
  it('compresses dashboard secondary cards and list headers on mobile without hiding actions or search', () => {
    ...
    expect(dashboard).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-task-copy--secondary\s*\{[\s\S]*display:\s*none/
    )
    expect(contentView).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*:deep\(\.list-heading\)\s*\{[\s\S]*display:\s*none/
    )
    expect(uploadsView).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*:deep\(\.list-heading\)\s*\{[\s\S]*display:\s*none/
    )
  })
  ...
  it('reduces mobile admin framing to a compact shell title and lighter page context strip', () => {
    ...
    expect(header).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-shell-title-desktop\s*\{[\s\S]*display:\s*none/
    )
    expect(header).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-shell-title-mobile\s*\{[\s\S]*display:\s*block/
    )
    ...
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-copy\s*:is\(h1,\s*h2,\s*h3\)\s*\{[\s\S]*font-size:\s*clamp\(1\.26rem,\s*6vw,\s*1\.62rem\)/
    )
    expect(css).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-page-header\s*\{[\s\S]*padding:\s*12px\s*12px\s*10px/
    )
  ...
  it('makes mobile list actions and system steps easier to operate without desktop-style wrapping', () => {
    ...
    expect(contentView).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*:deep\(\.item-actions\)\s*\{[\s\S]*display:\s*grid[\s\S]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/
    )
    expect(uploadsView).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*:deep\(\.item-actions\)\s*\{[\s\S]*display:\s*grid[\s\S]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/
    )
    expect(systemSteps).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.step-list\s*\{[\s\S]*display:\s*grid[\s\S]*grid-auto-flow:\s*column[\s\S]*overflow-x:\s*auto/
    )
  })
  ...
  it('turns mobile edit panels into dismissible bottom sheets instead of stacking editors after the list', () => {
    ...
    expect(contentView).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.editor-panel--sheet\s*\{[\s\S]*position:\s*fixed[\s\S]*bottom:\s*0/
    )
    expect(contentView).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.editor-panel--sheet\.is-open\s*\{[\s\S]*transform:\s*translateY\(0\)/
    )
    ...
    expect(contentEdit).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.editor-footer\s*\{[\s\S]*position:\s*sticky[\s\S]*bottom:\s*0/
    )
  ...
  it('turns mobile library and taxonomy management into low-noise sheets and action strips', () => {
    ...
    expect(libraryCss).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.library-mobile-primary-actions\s*\{[\s\S]*display:\s*grid[\s\S]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/
    )
    expect(libraryCss).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.library-mobile-sheet\s*\{[\s\S]*position:\s*fixed[\s\S]*bottom:\s*0/
    )
    expect(libraryCss).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.library-mobile-sheet\.is-open\s*\{[\s\S]*transform:\s*translateY\(0\)/
    )
    ...
    expect(taxonomy).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.taxonomy-mobile-actions\s*\{[\s\S]*display:\s*grid[\s\S]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/
    )
    expect(taxonomy).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.taxonomy-editor-sheet\s*\{[\s\S]*position:\s*fixed[\s\S]*bottom:\s*0/
    )
    expect(taxonomy).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.taxonomy-editor-sheet\.is-open\s*\{[\s\S]*transform:\s*translateY\(0\)/
    )
    expect(taxonomyGroup).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.actions\s*\{[\s\S]*position:\s*sticky[\s\S]*bottom:\s*0/
    )
    expect(taxonomyCategory).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.actions\s*\{[\s\S]*position:\s*sticky[\s\S]*bottom:\s*0/
    )
  })
  ...
  it('collapses secondary library list tools on mobile so filters and recycle content stop crowding the first screen', () => {
    ...
    expect(libraryCss).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.library-mobile-tools\s*\{[\s\S]*display:\s*grid/
    )
    expect(libraryCss).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.library-mobile-tools-body\s*\{[\s\S]*display:\s*none/
    )
    expect(libraryCss).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.library-mobile-tools-body\.is-open\s*\{[\s\S]*display:\s*grid/
    )
    expect(libraryCss).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.library-mobile-deleted-body\s*\{[\s\S]*display:\s*none/
    )
    expect(libraryCss).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.library-mobile-deleted-body\.is-open\s*\{[\s\S]*display:\s*grid/
    )
  })
  ...
  it('widens desktop admin work surfaces and compresses shell framing into a denser toolbar', () => {
    ...
    expect(foundation).toMatch(
      /@media\s*\(min-width:\s*1280px\)\s*\{[\s\S]*\.app-main--admin\s*\{[\s\S]*width:\s*min\(1360px,\s*calc\(100%\s*-\s*64px\)\)/
    )
    expect(topbar).toMatch(
      /@media\s*\(min-width:\s*1280px\)\s*\{[\s\S]*\.topbar--admin\s+\.topbar-inner\s*\{[\s\S]*width:\s*min\(1360px,\s*calc\(100%\s*-\s*64px\)\)/
    )
    expect(header).toMatch(
      /@media\s*\(min-width:\s*960px\)\s*\{[\s\S]*\.admin-shell-header\s*\{[\s\S]*align-items:\s*center[\s\S]*padding:\s*12px\s*18px/
    )
    expect(header).toMatch(
      /@media\s*\(min-width:\s*960px\)\s*\{[\s\S]*\.admin-shell-description\s*\{[\s\S]*display:\s*none/
    )
    expect(header).toMatch(
      /@media\s*\(min-width:\s*960px\)\s*\{[\s\S]*\.admin-shell-status-strip\s*\{[\s\S]*display:\s*flex[\s\S]*align-items:\s*center/
    )
  })
  ...
  it('keeps desktop shell actions inside a wrapped toolbar instead of letting the home link drift to the edge', () => {
    ...
    expect(header).toMatch(
      /\.admin-shell-toolbar\s*\{[\s\S]*display:\s*flex[\s\S]*flex-wrap:\s*wrap/
    )
    expect(header).toMatch(
      /@media\s*\(min-width:\s*960px\)\s*\{[\s\S]*\.admin-shell-toolbar\s*\{[\s\S]*justify-content:\s*flex-end/
    )
  })
})
