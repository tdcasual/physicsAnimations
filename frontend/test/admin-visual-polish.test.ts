...
describe('admin visual polish', () => {
  ...
  it('turns the mobile workspace menu into a bottom-sheet control deck', () => {
    ...
    expect(source).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.admin-nav-bar\s*\{[\s\S]*position:\s*fixed[\s\S]*left:\s*12px[\s\S]*right:\s*12px[\s\S]*bottom:/
    )
    expect(source).toMatch(
  ...
  it('turns mobile library and taxonomy management into low-noise sheets and action strips', () => {
    ...
    expect(taxonomy).toMatch(
      /@media\s*\(max-width:\s*640px\)\s*\{[\s\S]*\.taxonomy-editor-sheet\s*\{[\s\S]*position:\s*fixed[\s\S]*bottom:/
    )
    expect(taxonomy).toMatch(
  ...
  it('widens desktop admin work surfaces and compresses shell framing into a denser toolbar', () => {
    ...
    expect(header).toMatch(
      /@media\s*\(min-width:\s*1024px\)\s*\{[\s\S]*\.admin-shell-header\s*\{[\s\S]*align-items:\s*center[\s\S]*padding:\s*12px\s*18px/
    )
    expect(header).toMatch(
      /@media\s*\(min-width:\s*1024px\)\s*\{[\s\S]*\.admin-shell-description\s*\{[\s\S]*display:\s*none/
    )
    expect(header).toMatch(
      /@media\s*\(min-width:\s*1024px\)\s*\{[\s\S]*\.admin-shell-status-strip\s*\{[\s\S]*display:\s*flex[\s\S]*align-items:\s*center/
    )
  })
  it('keeps desktop shell actions inside a wrapped toolbar instead of letting the home link drift to the edge', () => {
    ...
    expect(header).toMatch(
      /@media\s*\(min-width:\s*1024px\)\s*\{[\s\S]*\.admin-shell-toolbar\s*\{[\s\S]*justify-content:\s*flex-end/
    )
  })
})
