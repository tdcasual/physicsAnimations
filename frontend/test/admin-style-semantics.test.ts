...
describe('admin style semantics', () => {
  ...
  it('applies shared classes in core admin views', () => {
    ...
    expect([content, contentCreate, contentEdit].some(source => /admin-actions/.test(source))).toBe(
      true
    )
    expect([uploads, uploadsCreate, uploadsEdit].some(source => /admin-actions/.test(source))).toBe(
      true
    )
    ...
    expect(
      [content, contentCreate, contentEdit].some(source => /admin-feedback/.test(source))
    ).toBe(true)
    expect(
      [uploads, uploadsCreate, uploadsEdit].some(source => /admin-feedback/.test(source))
    ).toBe(true)
