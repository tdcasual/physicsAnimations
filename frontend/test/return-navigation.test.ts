...
import {
  resolveBackNavigationMode,
  resolveBackNavigationTarget,
} from '../src/features/navigation/backNavigation'
...
describe('return navigation behavior', () => {
  ...
  it('supports section-aware fallback targets when history back is unavailable', () => {
    expect(
      resolveBackNavigationTarget({ historyState: null, fallbackHash: '#catalog-recent' })
    ).toEqual({
      mode: 'replace-home',
      path: '/',
      hash: '#catalog-recent',
    ...
    expect(
      resolveBackNavigationTarget({
        historyState: { back: '/' },
        fallbackHash: '#catalog-favorites',
      })
    ).toEqual({
      mode: 'history-back',
      path: '',
