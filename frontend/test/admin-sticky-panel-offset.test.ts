...
function expectStickyPanelGuardrails(source: string) {
  ...
  expect(source).toMatch(/\.editor-panel\s*\{[\s\S]*top:\s*calc\(var\(--app-topbar-height,\s*0px\)\s*\+\s*12px\)/);
  expect(source).toMatch(/\.editor-panel\s*\{[\s\S]*max-height:\s*calc\(100dvh\s*-\s*var\(--app-topbar-height,\s*0px\)\s*-\s*32px\)/);
  ...
  expect(source).toMatch(/@media\s*\(max-width:\s*1024px\)\s*\{[\s\S]*\.editor-panel\s*\{[\s\S]*position:\s*static/);
  expect(source).toMatch(/@media\s*\(max-width:\s*1024px\)\s*\{[\s\S]*\.editor-panel\s*\{[\s\S]*max-height:\s*none/);
  expect(source).toMatch(/@media\s*\(max-width:\s*1024px\)\s*\{[\s\S]*\.editor-panel\s*\{[\s\S]*overflow:\s*visible/);
}
