...
describe('viewer screenshot layout', () => {
  ...
  it('lets the screenshot image define the stage height in screenshot mode', () => {
    ...
    expect(source).toMatch(
      /\.viewer-stage-frame--screenshot \.viewer-shot\s*\{[\s\S]*position:\s*static/
    )
    expect(source).toMatch(
      /\.viewer-stage-frame--screenshot \.viewer-shot\s*\{[\s\S]*height:\s*auto/
    )
  })
  ...
  it('uses a compact stage frame with tighter spacing on phones', () => {
    ...
    expect(source).toMatch(
      /@media \(max-width: 640px\)\s*\{[\s\S]*\.viewer-stage-frame--priority\s*\{[\s\S]*padding:\s*10px/
    )
  })
})
