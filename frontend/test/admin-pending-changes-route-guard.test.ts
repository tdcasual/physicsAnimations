...
describe("admin pending changes route guard", () => {
  ...
  it("wires the shared guard into mutable admin views with page-specific messages", () => {
    ...
    expect(content).toMatch(/usePendingChangesGuard\(\{[\s\S]*hasPendingChanges:\s*hasPendingEditChanges[\s\S]*isBlocked:\s*saving[\s\S]*message:\s*"当前编辑内容有未保存更改，确定离开当前页面吗？"/);
    expect(uploads).toMatch(/usePendingChangesGuard\(\{[\s\S]*hasPendingChanges:\s*hasPendingEditChanges[\s\S]*isBlocked:\s*saving[\s\S]*message:\s*"当前编辑内容有未保存更改，确定离开当前页面吗？"/);
    expect(taxonomy).toMatch(/usePendingChangesGuard\(\{[\s\S]*hasPendingChanges[\s\S]*isBlocked:\s*saving[\s\S]*message:\s*"分类内容有未保存更改，确定离开当前页面吗？"/);
    expect(account).toMatch(/usePendingChangesGuard\(\{[\s\S]*hasPendingChanges[\s\S]*isBlocked:\s*saving[\s\S]*message:\s*"账号信息有未保存更改，确定离开当前页面吗？"/);
    expect(library).toMatch(/usePendingChangesGuard\(\{[\s\S]*hasPendingChanges[\s\S]*message:\s*"资源库内容有未保存更改，确定离开当前页面吗？"/);
  });
